...
export const addSales = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const managerEmail = await BusinessManager.findOne({ email });
    if (managerEmail) {
      return res.status(400).json({ error: "Email already registered" });
    }
...
export const getBusinessSales = async (req: Request, res: Response) => {
  const id = req.params.id;
  try {
    const data = await Sales.find({ businessId: id });
    if (data.length === 0) {
...
export const updateBusinessSales = async (req: Request, res: Response) => {
